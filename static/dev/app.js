...
function insertLoadingGif(){
  ...
  //get paremeter from URL      --> ex.?category=categories
  //if that param doesn't exist --> set to "none"
function getState(){
  ...
  var pickattr = findGetParameter("pickattr");    if(pickattr.length === undefined) {pickattr = ""}          logger("found pickattr", attr)  
  ...
  var lang = findGetParameter("lang");         if(lang.length === undefined) {lang = "ENG"}            logger("found language", lang)
...
function stateMenuHandle(state,lang){//input = arr = ["string","string"]/ lang = String

    //default = 9
  var _case = 9;
  var UI;

    //becareful about load order
    //load source form different location
    //
    if(state[0]=="end"){
          _case = 3;
          ...
          _case = 1
      } else if(state[0]=="category"||"brand"||"attr"){//all of them are on the same location
          _case = 0;
    }
...
//#############################################################################
function setup() {
  logger("setup","Constructing Page");
  showButtons();
}
